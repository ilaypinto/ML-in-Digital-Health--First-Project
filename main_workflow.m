...
good_labels_folder = [16 20	23	24	31	36	37	38	39	41	42	44	45	46 ...
                     47	49	50	51	52	53	54	55	57	58	59	60	73	74 ...
                 	 75	76	77	78	79	80	81	82	83	84	87	90	91	93 ...
                 	 94	159	160	161	163	164	165	166	167	168	173	174	175	176 ...
                 	 178    182	183	184	185	186	187	188	189	190	191	192	205	...
                     206	208	213	214	215	216	227	241	242	243	244	245	246	...
                     247	248	249	250	251	252	253	254	256	257	258	259	260	...
                     262	265	266	267	269	270	271	272	273	274	276	277	279	...
                     280	281	282	283	284	289	293	294	295	296	297	298	299	...
                     300];  % cherry pick folders with good labeled recordings
...
data = data_from_csv(folders, flag_data_csv, label_time, 1);

%% create train and test set for MW and ET segmentation
folders_train = [16 20	23	24	31	36	37	38	39	41	42	44	45	46 ...
                     47	49	50	51	52	53	54	55	57	58	59	60	73	74 ...
                 	 75	76	77	78	79	80	81	82	83	84	87	90	91	93 ...
                 	 94	159	160	161	163	164	165	166	167	168	173	174	175	176 ...
                 	 178 205	206	208	213	214	215	216	227	241	242	243	244	245	246	...
                     247	248	249	250	251	252	253	254	256	257	258	259	260	...
                     262	265	266	267	269	270	271	272	273	274	276	277	279	...
                     280	281	282	283	284	];         
folders_test = [182	183	184	185	186	187	188	189	190	191	192 289	293	294	295	296	297	298	299	300];   % try cherry pick with good labeled recordings

MW_train_set = create_data_set(folders_train, data, segmentation(1,:), overlap, 'train', label_time, flag_segm_MW,1);
MW_test_set = create_data_set(folders_test, data, segmentation(1,:), overlap, 'test', label_time, flag_segm_MW,1);

ET_train_set = create_data_set(folders_train, data, segmentation(2,:), overlap, 'train', label_time, flag_segm_ET,1);
ET_test_set = create_data_set(folders_test, data, segmentation(2,:), overlap, 'test', label_time, flag_segm_ET,1);

%% extract features from segmentations - MW & ET 
% ############# need to add a random algo for reproducibility#############
MW_train_feat = create_train_feat_set(MW_train_set, 'MW_train_feat', flag_feat_MW, labels_tags);
MW_test_feat = create_test_feat_set(MW_test_set, 'MW_test_feat', flag_feat_MW , labels_tags);

ET_train_feat = create_train_feat_set(ET_train_set, 'ET_train_feat', flag_feat_ET, labels_tags);
ET_test_feat = create_test_feat_set(ET_test_set, 'ET_test_feat', flag_feat_ET, labels_tags);
...
final_ET_feat_train = ET_new_train_feat(:, [Indx_ET size(ET_new_train_feat,2)]);
final_ET_feat_test = ET_new_test_feat(:, [Indx_ET size(ET_new_test_feat,2)]);
